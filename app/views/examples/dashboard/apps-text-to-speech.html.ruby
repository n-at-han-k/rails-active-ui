# Shadcn UI Kit Dashboard — Text to Speech
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "volume up", inverted: true); LinkTo(href: "/examples/dashboard/apps-text-to-speech", style: "color: inherit; text-decoration: none;") { "Text to Speech" } }
    }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Text to Speech" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        Card(fluid: true, centered: true, style: "max-width: 400px; margin: 4em auto;") { |c|
          c.header {
            Loader(active: true, inline: true, centered: true, size: "large")
          }
          c.description {
            Header(size: :h3, aligned: "center") { "Text to Speech" }
            Text(size: "sm", color: "grey", style: "text-align: center; display: block;") { "Convert your text into natural-sounding speech using AI-powered voice synthesis." }
            Divider(hidden: true)
            Message(type: "warning", icon: "exclamation triangle", compact: true) { |m|
              m.header { "Under Construction" }
              text "This feature is currently being developed. Check back soon!"
            }
          }
        }
