# Shadcn UI Kit Dashboard — Social Media
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "heart", inverted: true); LinkTo(href: "/examples/dashboard/apps-social-media", style: "color: inherit; text-decoration: none;") { "Social Media" }; Wrapper(class: "nav-badge new") { "New" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Social Media" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        Card(fluid: true) { |c|
          c.description {
            HStack(spacing: 3, align: "center", style: "margin-bottom: 0.75em;") {
              Image(src: "/images/shadcn-dashboard/avatars/1.jpg", circular: true, size: "tiny")
              Wrapper {
                Text(weight: "semibold") { "Toby Belhome" }
                Text(size: "xs", color: "grey") { "@tobybelhome" }
              }
            }
            Input(icon: "search", icon_position: "left", placeholder: "Search...", fluid: true, size: "mini")
            Divider(hidden: true)
            List(selection: true, divided: true) {
              [["home", "Home", true], ["clipboard list", "Tasks", false], ["users", "Users", false], ["globe", "APIs", false], ["credit card", "Subscription", false], ["cog", "Settings", false], ["help circle", "Help", false]].each do |icon, label, active|
                ListItem(active: active) { Icon(name: icon); text " #{label}" }
              end
            }
            Divider()
            Button(color: "blue", fluid: true, icon: "pencil") { " Create Post" }
            Divider(hidden: true)
            Message(type: "warning", compact: true, dismissible: true, size: "mini") { |m|
              m.header { "Upgrade to Pro" }
              text "Get unlimited posts and analytics."
            }
          }
        }
        ...
        Card(fluid: true) { |c|
          c.header {
            # Stories row
            HStack(spacing: 6) {
              [["1", "Your Story", true], ["2", "Jackson", false], ["3", "Hally", false], ["4", "Sofia", false], ["1", "Carlos", false], ["2", "Emma", false]].each do |av, name, is_self|
                Wrapper(style: "text-align: center;") {
                  Image(src: "/images/shadcn-dashboard/avatars/#{av}.jpg", circular: true, bordered: true, style: "width: 48px; height: 48px;")
                  Text(size: "xs") { name }
                }
              end
            }
          ...
          c.description {
            ...
            Feed(size: "small") {
              [
                ["1", "Toby Belhome", "2h", "/images/shadcn-dashboard/products/01.jpeg", "Just launched our new product! Check it out and let me know what you think.", 24, 8],
                ["2", "Jackson Lee", "5h", nil, "Working on some exciting new features for the dashboard. Stay tuned for updates! The team has been putting in incredible work this sprint.", 18, 5],
                ["3", "Hally Gray", "1d", "/images/shadcn-dashboard/products/03.jpeg", "Beautiful sunset from the office rooftop. Sometimes you need to step back and appreciate the view.", 42, 12],
              ].each do |av, name, time, img, caption, likes, comments|
                FeedItem { |e|
                  ...
                  e.summary {
                    HStack(spacing: 3, justify: "between", align: "center") {
                      ...
                      Dropdown(button: true, floating: true, compact: true, pointing: "top right", class: "icon basic mini") {
                        ...
                        MenuMenu(class: "menu") {
                          MenuItem(icon: "eye slash") { "Hide" }
                          MenuItem(icon: "flag") { "Report" }
                          MenuItem(icon: "user times") { "Unfollow" }
                          MenuItem(icon: "link") { "Copy link" }
                        }
                      }
                    }
                  }
                  e.extra_text { caption }
                  if img
                    e.extra_images { Image(src: img, fluid: true, rounded: true) }
                  end
                  e.meta {
                    HStack(spacing: 8, align: "center") {
                      HStack(spacing: 2, align: "center") { Icon(name: "heart outline", link: true); Text(size: "xs") { likes.to_s } }
                      HStack(spacing: 2, align: "center") { Icon(name: "comment outline", link: true); Text(size: "xs") { comments.to_s } }
                      Icon(name: "share", link: true)
                      Icon(name: "bookmark outline", link: true)
                    }
                  }
                }
              end
        ...
        # ── Suggestions sidebar ──────────────────────────────────
        Card(fluid: true) { |c|
          c.header { Header(size: :h5, style: "margin: 0;") { "Suggested for You" } }
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["1", "Sarah Johnson", "@sarahj"],
                ["2", "Mike Chen", "@mikechen"],
                ["3", "Emma Davis", "@emmad"],
                ["4", "Alex Kim", "@alexkim"],
                ["1", "Lisa Park", "@lisapark"],
                ["2", "Ryan Cooper", "@ryanc"],
              ].each do |av, name, handle|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
                      ...
                      Wrapper {
                        ...
                        Text(size: "xs", color: "grey") { handle }
                      }
                    }
                    Button(color: "blue", size: "mini") { "Follow" }
                  }
                }
              end
            }
