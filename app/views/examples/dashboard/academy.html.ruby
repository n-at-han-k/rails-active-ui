# Shadcn UI Kit Dashboard — Academy
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "graduation cap", inverted: true); LinkTo(href: "/examples/dashboard/academy", style: "color: inherit; text-decoration: none;") { "Academy" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Academy" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        # ── Row 1: Welcome (span 2) | Learning Paths ──────────────
        Wrapper(class: "span-2") {
          Card(fluid: true, color: "blue") { |c|
            c.description {
              Header(size: :h3, inverted: true) { "Hi, Andrew! Ready to learn?" }
              Text(size: "sm") { "You have 3 courses in progress. Keep up the great work!" }
              Divider(hidden: true)
              Button(inverted: true) { text "Continue Learning "; Icon(name: "arrow right") }
            }
          }
        }

        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Learning Path" } }
          c.description {
            List(divided: true, relaxed: true) {
              ListItem {
                HStack(spacing: 3, justify: "between", align: "center") {
                  HStack(spacing: 3, align: "center") { Icon(name: "code branch", color: "blue"); Text(weight: "semibold", size: "sm") { "Frontend Development" } }
                  Icon(name: "chevron right", color: "grey")
                }
                Progress(value: 72, total: 100, color: "blue", size: "tiny", style: "margin-top: 0.5em;") { |p| p.bar {}; p.label { "72%" } }
              }
              ListItem {
                HStack(spacing: 3, justify: "between", align: "center") {
                  HStack(spacing: 3, align: "center") { Icon(name: "code branch", color: "green"); Text(weight: "semibold", size: "sm") { "Backend Development" } }
                  Icon(name: "chevron right", color: "grey")
                }
                Progress(value: 45, total: 100, color: "green", size: "tiny", style: "margin-top: 0.5em;") { |p| p.bar {}; p.label { "45%" } }
              }
            }
          }
        }

        # ── Row 2: Student Success | Progress | Activity Chart ─────
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Student Success" } }
          c.description {
            Wrapper(class: "kpi-value", style: "text-align: center;") { "88%" }
            Text(size: "xs", color: "grey", style: "text-align: center; display: block;") { "Pass Rate" }
            Progress(value: 88, total: 100, color: "green", indicating: true, style: "margin-top: 0.75em;") { |p| p.bar {} }
            Divider(hidden: true)
            HStack(spacing: 8, justify: "between") {
              Wrapper { Icon(name: "users", color: "blue"); Text(size: "xs", color: "grey") { "Total" }; Text(weight: "bold") { "2,450" } }
              Wrapper { Icon(name: "check circle", color: "green"); Text(size: "xs", color: "grey") { "Passing" }; Text(weight: "bold") { "2,156" } }
            }
          }
        }

        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Progress Statistics" } }
          c.description {
            Wrapper(class: "kpi-value", style: "text-align: center;") { "72.5%" }
            Text(size: "xs", color: "grey", style: "text-align: center; display: block;") { "Overall Progress" }
            Divider(hidden: true)
            [["In Progress", 34, "blue", "clock"], ["Completed", 48, "green", "check circle"]].each do |label, count, color, icon|
              HStack(spacing: 3, align: "center", style: "margin-bottom: 0.5em;") {
                Icon(name: icon, color: color)
                Wrapper(style: "flex: 1;") {
                  HStack(spacing: 3, justify: "between") { Text(size: "sm") { label }; Text(weight: "semibold") { count.to_s } }
                  Progress(value: count, total: 82, color: color, size: "tiny") { |p| p.bar {} }
                }
              }
            end
          }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Most Activity" } }
          c.description {
            List(divided: true) {
              [["Mentoring", 45, "blue"], ["Organization", 30, "green"], ["Planning", 25, "orange"]].each do |label, pct, color|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
        ...
        # ── Row 3: Course Progress Chart (span 2) | Leaderboard ───
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Course Progress by Month" }
                Tag(color: "green", basic: true, size: "mini") { "+2.5%" }
              }
            }
            c.description {
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Leaderboard" } }
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["1", "Sarah Johnson", "2,450 pts"],
                ["2", "Mike Chen", "2,120 pts"],
                ["3", "Emma Davis", "1,890 pts"],
                ["4", "Alex Kim", "1,650 pts"],
              ].each_with_index do |(av, name, pts), i|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
                      Tag(circular: true, size: "mini", color: i == 0 ? "yellow" : nil) { (i + 1).to_s }
                      Image(src: "/images/shadcn-dashboard/avatars/#{av}.jpg", avatar: true)
                      Text(weight: "semibold", size: "sm") { name }
                    }
                    Tag(basic: true, size: "mini") { pts }
                  }
                }
              end
            }
        ...
        # ── Row 4: Courses Table (span 3) ──────────────────────────
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Courses" }
                Input(icon: "search", icon_position: "left", placeholder: "Search courses...", size: "mini")
              }
            }
            c.description {
              Table(basic: "very", size: "small", selectable: true) { |t|
                t.header {
                  TableRow {
                    TableCell(heading: true, width: 4) { "Course" }
                    TableCell(heading: true, width: 2) { "Rating" }
                    TableCell(heading: true, width: 3) { "Progress" }
                    TableCell(heading: true, width: 2) { "Status" }
                    TableCell(heading: true, width: 1) { "" }
                  }
                }
                [
                  ["react", "React Fundamentals",   4.8, 85, "In Progress", "blue"],
                  ["angular", "Angular Advanced",   4.5, 100, "Completed",  "green"],
                  ["vue", "Vue.js Mastery",         4.7, 62, "In Progress", "blue"],
                  ["html", "HTML & CSS Basics",     4.9, 100, "Completed",  "green"],
                  ["css", "Responsive Design",      4.3, 45, "In Progress", "blue"],
                  ["svelte", "Svelte Workshop",     4.6, 20, "New",         "teal"],
                ].each do |tech, name, rating, pct, status, color|
                  TableRow {
                    TableCell {
                      HStack(spacing: 3, align: "center") {
                        Image(src: "/images/shadcn-dashboard/tech/#{tech}.svg", size: "mini")
                        Text(weight: "semibold") { name }
                      }
                    }
                    TableCell {
                      HStack(spacing: 2, align: "center") { Icon(name: "star", color: "yellow", size: "small"); Text(size: "sm") { rating.to_s } }
                    }
                    ...
                    TableCell { Button(class: "basic mini") { pct < 100 ? "Continue" : "Review" } }
                  }
                end
              }
            }
        ...
        Card(fluid: true) { |c|
          c.description {
            Text(size: "sm", color: "grey") { "Conversion Rate" }
            Wrapper(class: "kpi-value") { "3.2%" }
            Tag(color: "green", basic: true, size: "mini") { Icon(name: "arrow up"); text "+0.5%" }
          }
        }
