# Shadcn UI Kit Dashboard — Crypto Dashboard
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "bitcoin", inverted: true); LinkTo(href: "/examples/dashboard/crypto", style: "color: inherit; text-decoration: none;") { "Crypto" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Crypto Dashboard" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        # ── Row 1: Overview | Wallets | Trading ──────────────────────
        Card(fluid: true, color: "blue") { |c|
          c.description {
            Text(size: "sm", color: "grey") { "Total Balance" }
            Wrapper(class: "kpi-value") { "$46,200.00" }
            HStack(spacing: 8, align: "center", style: "margin-top: 0.5em;") {
              Tag(color: "green", size: "mini") { Icon(name: "arrow up"); text "+12%" }
              Text(size: "xs", color: "grey") { "150 Transactions" }
              Text(size: "xs", color: "grey") { "3 Wallets" }
            }
          }
        }

        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Digital Wallets" } }
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["/images/shadcn-dashboard/crypto-icons/bitcoin.svg", "Bitcoin", "BTC", "$28,450.00", "+5.2%", "green"],
                ["/images/shadcn-dashboard/crypto-icons/ethereum.svg", "Ethereum", "ETH", "$12,320.00", "+3.1%", "green"],
                ["/images/shadcn-dashboard/crypto-icons/avalanche.svg", "Avalanche", "AVAX", "$5,430.00", "-1.8%", "red"],
              ].each do |icon, name, sym, val, change, color|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
                      Image(src: icon, size: "mini")
                      Wrapper { Text(weight: "semibold") { name }; Text(size: "xs", color: "grey") { sym } }
                    }
        ...
        Card(fluid: true) { |c|
          c.header {
            HStack(spacing: 3, justify: "between", align: "center") {
              Header(size: :h4, style: "margin: 0;") { "Quick Trade" }
              Wrapper(style: "display: flex; gap: 0.35em;") {
                Button(color: "green", size: "mini") { "Buy" }
                Button(class: "basic mini") { "Sell" }
              }
            }
          }
          c.description {
            Form(size: "small") {
              Field(label: "Coin") {
                Dropdown(selection: true, default_value: "btc", fluid: true) {
                  MenuItem(value: "btc") { "Bitcoin (BTC)" }
                  MenuItem(value: "eth") { "Ethereum (ETH)" }
                  MenuItem(value: "avax") { "Avalanche (AVAX)" }
                  MenuItem(value: "sol") { "Solana (SOL)" }
                }
              }
              Field(label: "Amount (USD)") {
                Input(placeholder: "0.00", fluid: true, icon: "dollar sign", icon_position: "left")
              }
              Field(label: "You receive") {
                Input(placeholder: "0.00000", fluid: true, disabled: true)
              }
            }
          }
          c.extra {
            Button(color: "green", fluid: true) { "Buy Bitcoin" }
          }
        }

        # ── Row 2: Recent Activities | Balance Chart (span 2) ──────
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Recent Activities" } }
          c.description {
            Feed(size: "small") {
              [
                ["bitcoin", "Bought Bitcoin", "+0.0045 BTC", "$245.00", "green", "2 hours ago"],
                ["ethereum", "Sold Ethereum", "-0.52 ETH", "$1,240.00", "red", "5 hours ago"],
                ["bitcoin", "Received Bitcoin", "+0.012 BTC", "$650.00", "blue", "Yesterday"],
                ["avalanche", "Bought Avalanche", "+15 AVAX", "$320.00", "green", "2 days ago"],
                ["ethereum", "Bought Ethereum", "+0.8 ETH", "$1,920.00", "green", "3 days ago"],
                ["bitcoin", "Sent Bitcoin", "-0.005 BTC", "$270.00", "red", "4 days ago"],
              ].each do |coin, action, qty, val, color, time|
                FeedItem { |e|
                  e.label { Image(src: "/images/shadcn-dashboard/crypto-icons/#{coin}.svg") }
                  e.summary {
                    Text(weight: "semibold") { action }
                    text " "
                    Tag(color: color, basic: true, size: "mini") { qty }
                  }
                  e.date { time }
                  e.meta { Text(weight: "semibold", size: "sm") { val } }
                }
              end
            }
          }
        ...
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            ...
            c.description {
              HStack(spacing: 12, justify: "start", align: "center", style: "margin-bottom: 1em;") {
                [["Received", "$32,400", "green"], ["Sent", "$18,200", "red"], ["Fees", "$450", "grey"]].each do |label, val, color|
                  Wrapper {
                    Text(size: "xs", color: "grey") { label }
                    Text(weight: "bold", color: color) { val }
                  }
                end
              }
