# Shadcn UI Kit Dashboard — Hospital Management
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "heartbeat", inverted: true); LinkTo(href: "/examples/dashboard/hospital-management", style: "color: inherit; text-decoration: none;") { "Hospital" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Hospital Management" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        # ── Row 1: Stat Cards ─────────────────────────────────────────
        [
          ["calendar check", "blue", "Appointments", "1,245"],
          ["user plus", "green", "New Patients", "328"],
          ["medkit", "purple", "Operations", "86"],
        ].each do |icon, color, label, val|
          Card(fluid: true) { |c|
            c.description {
        ...
        # ── Row 2: Patient Visits Chart (span 2) | By Department ──
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Patient Visits" }
                Dropdown(selection: true, compact: true, default_value: "2026", class: "mini") {
            ...
            c.description {
              ...
              HStack(spacing: 8, style: "margin-top: 0.75em;") {
                [["Female", "blue"], ["Male", "green"], ["Children", "orange"]].each do |label, color|
                  HStack(spacing: 3, align: "center") { Icon(name: "circle", color: color, size: "tiny"); Text(size: "xs") { label } }
                end
              }
            }
          }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Patients by Department" } }
          c.description {
            List(divided: true, relaxed: true) {
              [["Cardiology", 320, "red", 35], ["Neurology", 280, "blue", 30], ["Oncology", 190, "purple", 20], ["Pediatrics", 140, "green", 15]].each do |dept, count, color, pct|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") { Icon(name: "circle", color: color, size: "tiny"); Text(size: "sm") { dept } }
                    Text(weight: "semibold", size: "sm") { "#{count} (#{pct}%)" }
                  }
                }
              end
            }
        ...
        # ── Row 3: Calendar | Notes | Top Treatment ────────────────
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Planned" } }
          c.description {
            Calendar(type: "date") {
              Input(icon: "calendar", icon_position: "left", placeholder: "Pick a date", fluid: true)
            }
            Divider(hidden: true)
            List(divided: true) {
              [["1", "Dr. Smith - Checkup", "9:00 AM"], ["2", "Dr. Lee - Surgery", "11:00 AM"], ["3", "Dr. Garcia - Follow-up", "2:30 PM"]].each do |av, title, time|
                ListItem {
                  HStack(spacing: 3, align: "center") {
        ...
        Card(fluid: true) { |c|
          c.header {
            HStack(spacing: 3, justify: "between", align: "center") {
              Header(size: :h4, style: "margin: 0;") { "Notes" }
          ...
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["Follow up with patient #1234 on medication", "May 14"],
                ["Review lab results for Ward B", "May 13"],
                ["Schedule MRI for patient #5678", "May 13"],
                ["Staff meeting at 3 PM tomorrow", "May 12"],
                ["Order supplies for OR-3", "May 12"],
                ["Update patient discharge forms", "May 11"],
                ["Consult with Dr. Rivera re: case #890", "May 10"],
              ].each do |note, date|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "start") {
                    Wrapper {
                    ...
                    Button(icon: "trash", class: "basic icon mini", size: "mini")
                  }
                }
              end
            }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Top Treatment" } }
          c.description {
            # Segmented bar
            Wrapper(style: "display: flex; height: 10px; border-radius: 5px; overflow: hidden; margin-bottom: 1em;") {
              text '<div style="width:30%;background:#2185d0"></div><div style="width:25%;background:#21ba45"></div><div style="width:20%;background:#f2711c"></div><div style="width:15%;background:#6435c9"></div><div style="width:10%;background:#db2828"></div>'.html_safe
            }
            List(divided: true) {
              [["General Checkup", 450, "blue"], ["Dental Care", 380, "green"], ["Cardiology", 310, "orange"], ["Orthopedics", 220, "purple"], ["Pediatrics", 150, "red"]].each do |treatment, count, color|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") { Icon(name: "circle", color: color, size: "tiny"); Text(size: "sm") { treatment } }
        ...
        # ── Row 4: Appointments Table (span 2) | Recent Patients ──
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Upcoming Appointments" }
            ...
            c.description {
              Table(basic: "very", size: "small", selectable: true) { |t|
                t.header {
                  TableRow {
                    TableCell(heading: true, width: 3) { "Patient" }
                    TableCell(heading: true, width: 3) { "Doctor" }
                    TableCell(heading: true, width: 2) { "Department" }
                ...
                [
                  ["John Smith",    "Dr. Williams", "Cardiology",  "May 15", "Confirmed", "green"],
                  ["Emma Wilson",   "Dr. Lee",      "Neurology",   "May 15", "Pending",   "yellow"],
                  ["Carlos Rivera", "Dr. Garcia",   "Orthopedics", "May 16", "Confirmed", "green"],
                  ["Sarah Lee",     "Dr. Smith",    "Pediatrics",  "May 16", "Confirmed", "green"],
                  ["Alex Kim",      "Dr. Johnson",  "Oncology",    "May 17", "Pending",   "yellow"],
                  ["Maria Garcia",  "Dr. Williams", "Cardiology",  "May 17", "Cancelled", "red"],
                  ["David Brown",   "Dr. Lee",      "Neurology",   "May 18", "Confirmed", "green"],
                ].each do |patient, doctor, dept, date, status, color|
                  TableRow {
                    TableCell { Text(weight: "semibold") { patient } }
                    TableCell { doctor }
                    TableCell { dept }
                    ...
                    TableCell {
                      Dropdown(button: true, floating: true, compact: true, pointing: "top right", class: "icon basic mini") {
                        ...
                        MenuMenu(class: "menu") { MenuItem(icon: "eye") { "View" }; MenuItem(icon: "edit") { "Reschedule" } }
                      }
                    }
                  }
                end
        ...
        Card(fluid: true) { |c|
          ...
          c.description {
            List(divided: true, relaxed: true) {
              [["1","John Smith","Cardiology","May 14"], ["2","Emma Wilson","Neurology","May 13"], ["3","Carlos Rivera","Orthopedics","May 12"], ["4","Sarah Lee","Pediatrics","May 11"], ["1","Alex Kim","Oncology","May 10"]].each do |av, name, dept, date|
                ListItem {
                  HStack(spacing: 3, align: "center") {
                    Image(src: "/images/shadcn-dashboard/avatars/#{av}.jpg", avatar: true)
                    Wrapper {
                      Text(weight: "semibold", size: "sm") { name }
                      Text(size: "xs", color: "grey") { "#{dept} \u00B7 #{date}" }
                    }
                  }
                }
              end
