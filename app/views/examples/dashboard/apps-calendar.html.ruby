# Shadcn UI Kit Dashboard — Calendar
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "calendar", inverted: true); LinkTo(href: "/examples/dashboard/apps-calendar", style: "color: inherit; text-decoration: none;") { "Calendar" } }
    }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Calendar" }
      ...
      Wrapper(class: "dash-grid") {
          ...
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                HStack(spacing: 3, align: "center") {
                  Button(icon: "chevron left", class: "basic icon mini")
                  Header(size: :h4, style: "margin: 0;") { "May 2026" }
                  Button(icon: "chevron right", class: "basic icon mini")
                }
                HStack(spacing: 3) {
                  Button(class: "basic mini") { "Today" }
                  Dropdown(selection: true, compact: true, default_value: "month", class: "mini") {
                    MenuItem(value: "month") { "Month" }
                    MenuItem(value: "week") { "Week" }
                    MenuItem(value: "day") { "Day" }
                    MenuItem(value: "agenda") { "Agenda" }
                  }
                  Button(color: "blue", size: "mini", icon: "plus") { " New Event" }
                }
              }
            }
            c.description {
              ...
              Divider()
              # Agenda view for today
              Header(size: :h5) { "Today's Events" }
              List(divided: true, relaxed: true) {
                [
                  ["sky",     "9:00 AM - 10:00 AM",  "Team Standup",            "Daily sync meeting"],
                  ["violet",  "11:00 AM - 12:00 PM", "Design Review",           "Review new dashboard mockups"],
                  ["rose",    "1:00 PM - 2:00 PM",   "Client Call",             "Q2 progress update with Acme Corp"],
                  ["emerald", "3:00 PM - 4:00 PM",   "Sprint Planning",         "Plan sprint 24 backlog"],
                  ["amber",   "4:30 PM - 5:00 PM",   "1:1 with Manager",        "Weekly check-in"],
                ].each do |color_name, time, title, desc|
                  fui_color = {"sky" => "blue", "violet" => "purple", "rose" => "red", "emerald" => "green", "amber" => "orange"}[color_name]
                  ListItem {
                    HStack(spacing: 3, align: "start") {
                      Icon(name: "circle", color: fui_color, size: "tiny", style: "margin-top: 0.3em;")
                      Wrapper {
                        Text(weight: "semibold", size: "sm") { title }
                        Text(size: "xs", color: "grey") { time }
                        Text(size: "xs", color: "grey") { desc }
                      }
                    }
                  }
                end
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Upcoming" } }
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["blue",   "May 16", "Product Launch",      "All day"],
                ["green",  "May 18", "Board Meeting",       "10:00 AM"],
                ["orange", "May 20", "Company Offsite",     "All day"],
                ["purple", "May 22", "Tech Talk: AI/ML",    "2:00 PM"],
                ["red",    "May 25", "Deadline: Q2 Report", "EOD"],
              ].each do |color, date, title, time|
                ListItem {
                  HStack(spacing: 3, align: "start") {
                    Wrapper(style: "text-align: center; min-width: 45px;") {
                      Text(size: "xs", color: "grey") { date.split(" ").first }
                      Text(weight: "bold", size: "lg") { date.split(" ").last }
                    }
                    Wrapper {
                      Text(weight: "semibold", size: "sm") { title }
                      Text(size: "xs", color: "grey") { time }
            ...
            Divider()
            # Quick add
            Header(size: :h5) { "Quick Add" }
            Form(size: "small") {
              Input(placeholder: "Event title...", fluid: true)
              HStack(spacing: 3, style: "margin-top: 0.5em;") {
                Calendar(type: "date") {
                  Input(icon: "calendar", icon_position: "left", placeholder: "Date", size: "small")
                }
                Dropdown(selection: true, placeholder: "Color", compact: true, class: "mini") {
                  [["Blue", "blue"], ["Green", "green"], ["Red", "red"], ["Purple", "purple"], ["Orange", "orange"]].each do |label, val|
                    MenuItem(value: val) { Icon(name: "circle", color: val); text " #{label}" }
                  end
                }
              }
              Button(color: "blue", fluid: true, size: "small", style: "margin-top: 0.5em;") { "Add Event" }
            }
          }
        }
