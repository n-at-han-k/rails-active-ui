# Shadcn UI Kit Dashboard — Real Estate Dashboard
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "home", inverted: true); LinkTo(href: "/examples/dashboard/real-estate", style: "color: inherit; text-decoration: none;") { "Real Estate" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Real Estate Dashboard" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        # ── Row 1: Stat Cards ─────────────────────────────────────────
        [
          ["users", "blue", "Total Clients", "1,245"],
          ["dollar sign", "green", "Revenue", "$2.4M"],
          ["building", "purple", "Properties", "348"],
        ...
        # ── Row 2: Performance Chart (span 2) | Featured Property ──
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Performance" }
                Wrapper(style: "display: flex; gap: 0.35em;") {
                  Button(class: "basic mini") { "W" }; Button(class: "basic mini active") { "M" }; Button(class: "basic mini") { "Y" }
                }
              }
            }
            c.description {
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Featured Property" } }
          c.description {
            Image(src: "/images/shadcn-dashboard/products/01.jpeg", fluid: true, rounded: true)
            Divider(hidden: true)
            Header(size: :h5, style: "margin: 0;") { "Luxury Villa" }
            Text(size: "sm", color: "grey") { "123 Ocean Drive, Miami, FL" }
            Divider(hidden: true)
            HStack(spacing: 8, justify: "between") {
              Wrapper { Text(size: "xs", color: "grey") { "Sold" }; Text(weight: "bold") { "12" } }
              Wrapper { Text(size: "xs", color: "grey") { "Rented" }; Text(weight: "bold") { "8" } }
              Wrapper { Text(size: "xs", color: "grey") { "Views" }; Text(weight: "bold") { "2.4K" } }
            }
          }
        }

        # ── Row 3: Deals Progress | Leads | Sales Analytics ────────
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Deals Progress" } }
          c.description {
            HStack(spacing: 3, justify: "between", style: "margin-bottom: 0.5em;") {
              Wrapper { Text(size: "xs", color: "grey") { "Closed" }; Text(weight: "bold") { "68" } }
              Wrapper(style: "text-align: right;") { Text(size: "xs", color: "grey") { "In Progress" }; Text(weight: "bold") { "32" } }
            }
            Progress(value: 68, total: 100, color: "green") { |p| p.bar {}; p.label { "68 of 100 deals closed" } }
            Divider(hidden: true)
            List(divided: true) {
              [["Reminder: Follow up with buyer", "May 15", "1"], ["Site visit scheduled", "May 16", "2"], ["Contract review pending", "May 18", "3"]].each do |title, date, av|
                ListItem {
                  HStack(spacing: 3, align: "center") {
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Leads" } }
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["1", "Alice Morgan", "Buyer", "phone"],
                ["2", "Bob Chen", "Seller", "mail"],
                ["3", "Carol White", "Investor", "phone"],
                ["4", "Dan Rivera", "Buyer", "mail"],
              ].each do |av, name, type, contact_icon|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
                      Image(src: "/images/shadcn-dashboard/avatars/#{av}.jpg", avatar: true)
                      Wrapper {
                        ...
                        Tag(size: "mini", basic: true) { type }
                      }
                    }
                    Button(icon: contact_icon, class: "basic icon mini circular")
                  }
                }
              end
            }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Sales Analytics" } }
          c.description {
            Wrapper(class: "kpi-value", style: "text-align: center;") { "$1.8M" }
            Text(size: "xs", color: "grey", style: "text-align: center; display: block;") { "Total Sales" }
            Divider(hidden: true)
            List(divided: true) {
              [["Residential", "$820K", "blue"], ["Commercial", "$540K", "green"], ["Land", "$280K", "orange"], ["Rental", "$160K", "purple"]].each do |cat, val, color|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") { Icon(name: "circle", color: color, size: "tiny"); Text(size: "sm") { cat } }
        ...
        # ── Row 4: Active Listings (span 2) | Calendar ─────────────
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Active Listings" }
                Input(icon: "search", icon_position: "left", placeholder: "Search...", size: "mini")
              }
            }
            c.description {
              Table(basic: "very", size: "small", selectable: true) { |t|
                t.header {
                  TableRow {
                    TableCell(heading: true, width: 4) { "Property" }
                    TableCell(heading: true, width: 2) { "Type" }
                    TableCell(heading: true, width: 2) { "Price" }
                    TableCell(heading: true, width: 2) { "Status" }
                    TableCell(heading: true, width: 2) { "Agent" }
                  }
                }
                [
                  ["Oceanfront Condo", "Residential", "$1.2M", "Active",  "green"],
                  ["Downtown Office",  "Commercial",  "$3.5M", "Active",  "green"],
                  ["Suburban House",   "Residential", "$450K", "Pending", "yellow"],
                  ["Retail Space",     "Commercial",  "$890K", "Active",  "green"],
                  ["Lakefront Lot",    "Land",        "$180K", "Sold",    "blue"],
                ].each_with_index do |(name, type, price, status, color), i|
                  TableRow {
                    TableCell {
                      HStack(spacing: 3, align: "center") {
                        Image(src: "/images/shadcn-dashboard/products/0#{i+1}.jpeg", size: "mini", rounded: true)
                        Text(weight: "semibold") { name }
                      }
                    }
                    TableCell { Tag(size: "mini", basic: true) { type } }
                    TableCell { Text(weight: "semibold") { price } }
                    TableCell { Tag(size: "mini", color: color, basic: true) { status } }
                    TableCell { Image(src: "/images/shadcn-dashboard/avatars/#{(i % 4) + 1}.jpg", avatar: true) }
                  }
                end
              }
            }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Schedule" } }
          c.description {
            Calendar(type: "date") {
            ...
            Divider(hidden: true)
            List(divided: true) {
              [["Site visit - Oceanfront", "10:00 AM", "blue"], ["Client meeting", "2:00 PM", "green"], ["Contract signing", "4:30 PM", "orange"]].each do |title, time, color|
                ListItem {
                  HStack(spacing: 3, align: "center") {
