# Shadcn UI Kit Dashboard — Kanban Board
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "columns", inverted: true); LinkTo(href: "/examples/dashboard/apps-kanban", style: "color: inherit; text-decoration: none;") { "Kanban" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "Kanban Board" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        # ── Kanban Columns (3 columns side by side) ──────────────────
        [
          ["Backlog", "grey", [
            ["Research competitors", "Analyze top 5 competitor dashboards", "high", "1", 3, 2],
            ["Design system audit", "Review existing components for consistency", "medium", "2", 1, 5],
            ["API documentation", "Write OpenAPI specs for all endpoints", "low", "3", 0, 1],
            ["Accessibility review", "WCAG 2.1 compliance check", "high", "4", 2, 3],
          ]],
          ["In Progress", "blue", [
            ["User auth flow", "Implement OAuth2 with refresh tokens", "high", "1", 4, 8],
            ["Dashboard widgets", "Build stat cards and chart components", "medium", "2", 2, 4],
            ["Mobile responsive", "Fix layout issues on tablet/mobile", "medium", "3", 1, 2],
          ]],
          ["Done", "green", [
            ["Database schema", "Design and migrate production database", "high", "4", 0, 12],
            ["CI/CD pipeline", "Set up GitHub Actions with staging deploy", "medium", "1", 0, 6],
          ]],
        ].each do |col_title, col_color, tasks|
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                HStack(spacing: 3, align: "center") {
                  Header(size: :h5, style: "margin: 0;") { col_title }
                  Tag(circular: true, size: "mini") { tasks.length.to_s }
                }
                Button(icon: "plus", class: "basic icon mini")
              }
            }
            c.description {
              tasks.each do |title, desc, priority, av, attachments, comments|
                Card(fluid: true, raised: true, style: "margin-bottom: 0.5em;") { |tc|
                  tc.description {
                    Text(weight: "semibold", size: "sm") { title }
                    Text(size: "xs", color: "grey") { desc }
                    Divider(fitted: true, style: "margin: 0.5em 0;")
                    HStack(spacing: 3, justify: "between", align: "center") {
                      HStack(spacing: -4) {
                        (1..([2, av.to_i].min)).each { |i| Image(src: "/images/shadcn-dashboard/avatars/#{i}.jpg", avatar: true, style: "width: 22px; height: 22px; border: 2px solid #fff;") }
                      }
                      HStack(spacing: 6, align: "center") {
                        Tag(size: "mini", color: priority == "high" ? "red" : priority == "medium" ? "yellow" : "grey", basic: true) { priority.capitalize }
                        if attachments > 0
                          HStack(spacing: 2, align: "center") { Icon(name: "paperclip", size: "small", color: "grey"); Text(size: "xs", color: "grey") { attachments.to_s } }
                        end
                        HStack(spacing: 2, align: "center") { Icon(name: "comment outline", size: "small", color: "grey"); Text(size: "xs", color: "grey") { comments.to_s } }
                      }
                    }
                  }
                }
