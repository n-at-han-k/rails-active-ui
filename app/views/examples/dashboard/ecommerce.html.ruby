# Shadcn UI Kit Dashboard — E-Commerce Dashboard
...
Wrapper(class: "dash-layout") {
  ...
  Wrapper(class: "dash-sidebar") {
    ...
    Wrapper(class: "nav-section") {
      ...
      Wrapper(class: "nav-item active") { Icon(name: "shopping bag", inverted: true); LinkTo(href: "/examples/dashboard/ecommerce", style: "color: inherit; text-decoration: none;") { "E-commerce" } }
  ...
  Wrapper(class: "dash-main") {
    ...
    Wrapper(class: "dash-content") {
      ...
      Wrapper(class: "dash-title-bar") {
        Header(size: :h2, style: "margin: 0;") { "E-Commerce Dashboard" }
      ...
      Wrapper(class: "dash-grid") {
        ...
        Card(fluid: true, color: "blue") { |c|
          c.description {
            Header(size: :h3, inverted: true) { "Welcome back, Andrew!" }
            Text(size: "sm") { "Your sales are up 24% this month. Keep going!" }
            Divider(hidden: true)
            Wrapper(class: "kpi-value") { "$45,231" }
            Text(size: "xs", color: "grey") { "Monthly Revenue" }
          }
        }
        Card(fluid: true) { |c|
          c.description {
            Text(size: "sm", color: "grey") { "MRR" }
            Wrapper(class: "kpi-value") { "$12,450" }
            Tag(color: "green", basic: true, size: "mini") { Icon(name: "arrow up"); text "+12.5%" }
          }
        }
        Card(fluid: true) { |c|
          c.description {
            Text(size: "sm", color: "grey") { "Active Users" }
            Wrapper(class: "kpi-value") { "8,420" }
            Tag(color: "green", basic: true, size: "mini") { Icon(name: "arrow up"); text "+8.2%" }
          }
        }

        # ── Row 2: Revenue Chart (span 2) | Return Rate ───────────
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Total Revenue" }
                HStack(spacing: 6) {
                  Wrapper { Wrapper(class: "kpi-value", style: "font-size: 1.3em;") { "$28,450" }; Text(size: "xs", color: "grey") { "Desktop" } }
                  Wrapper { Wrapper(class: "kpi-value", style: "font-size: 1.3em;") { "$18,200" }; Text(size: "xs", color: "grey") { "Mobile" } }
                }
              }
            }
            c.description {
              Wrapper(class: "kpi-chart-placeholder", style: "height: 180px;") {
                [45, 62, 38, 72, 55, 68, 42, 78, 52, 65, 48, 70].each { |h| text "<div class=\"mini-bar\" style=\"height:#{h}%\"></div>".html_safe }
              }
            }
          }
        }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Return Rate" } }
          c.description {
            Wrapper(class: "mini-line-placeholder", style: "height: 180px;") {}
          }
        }

        # ── Row 3: Sales by Location | Reviews | Visit by Source ───
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Sales by Location" } }
          c.description {
            List(divided: true, relaxed: true) {
              [["us", "United States", "$12,450", "+8.2%", "green", 72],
               ["gb", "United Kingdom", "$8,320", "+5.1%", "green", 55],
               ["de", "Germany", "$6,180", "+3.4%", "green", 42],
               ["fr", "France", "$4,920", "-1.2%", "red", 35],
               ["jp", "Japan", "$3,750", "+6.8%", "green", 28],
               ["au", "Australia", "$2,890", "+2.1%", "green", 20],
              ].each do |flag, country, amount, change, color, pct|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
                      Flag(country: flag)
                      Wrapper {
                        Text(weight: "semibold", size: "sm") { country }
                        Progress(value: pct, total: 100, color: "blue", size: "tiny") { |p| p.bar {} }
                      }
                    }
                    Wrapper(style: "text-align: right;") {
                      Text(weight: "semibold", size: "sm") { amount }
                      Tag(color: color, basic: true, size: "mini") { change }
                    }
                  }
                }
              end
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Customer Reviews" } }
          c.description {
            Wrapper(class: "kpi-value", style: "text-align: center;") { "4.8" }
            HStack(spacing: 2, justify: "center") { 5.times { Icon(name: "star", color: "yellow") } }
            Text(size: "xs", color: "grey", style: "text-align: center; display: block; margin-top: 0.3em;") { "Based on 2,450 reviews" }
            Divider(hidden: true)
            [["5 stars", 78], ["4 stars", 15], ["3 stars", 5], ["2 stars", 1], ["1 star", 1]].each do |label, pct|
              HStack(spacing: 3, align: "center", style: "margin-bottom: 0.3em;") {
                Text(size: "xs", style: "width: 50px;") { label }
                Progress(value: pct, total: 100, color: "yellow", size: "tiny") { |p| p.bar {} }
        ...
        Card(fluid: true) { |c|
          c.header { Header(size: :h4, style: "margin: 0;") { "Visitors by Source" } }
          c.description {
            Wrapper(class: "kpi-value", style: "text-align: center;") { "10.2K" }
            Text(size: "xs", color: "grey", style: "text-align: center; display: block;") { "Total Visitors" }
            Divider(hidden: true)
            List(divided: true) {
              [["Direct", 3200, "blue"], ["Organic Search", 2800, "green"], ["Social Media", 2100, "purple"], ["Referral", 1400, "orange"], ["Email", 700, "teal"]].each do |src, count, color|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
        ...
        # ── Row 4: Recent Orders (span 2) | Best Selling (1 col) ──
        Wrapper(class: "span-2") {
          Card(fluid: true) { |c|
            c.header {
              HStack(spacing: 3, justify: "between", align: "center") {
                Header(size: :h4, style: "margin: 0;") { "Recent Orders" }
                Dropdown(button: true, floating: true, compact: true, pointing: "top right", class: "icon basic mini") {
                  Icon(name: "folder open")
                  text " Export"
                  MenuMenu(class: "menu") {
                    MenuItem(icon: "file excel") { "Excel" }
                    MenuItem(icon: "file pdf") { "PDF" }
                  }
                }
              }
            }
            c.description {
              Table(basic: "very", size: "small", selectable: true) { |t|
                t.header {
                  TableRow {
                    TableCell(heading: true, width: 3) { "Customer" }
                    TableCell(heading: true, width: 2) { "Product" }
                ...
                [
                  ["1", "Sarah Wilson",   "Wireless Headphones", "$89.99",  "Delivered",  "green",  "May 12"],
                  ["2", "Mike Johnson",   "Smart Watch Pro",     "$249.00", "Shipped",    "blue",   "May 11"],
                  ["3", "Emily Chen",     "Laptop Stand",        "$45.00",  "Delivered",  "green",  "May 10"],
                  ["4", "David Brown",    "USB-C Hub",           "$59.99",  "Processing", "yellow", "May 9"],
                  ["5", "Lisa Anderson",  "Mechanical Keyboard", "$129.00", "Shipped",    "blue",   "May 8"],
                  ["6", "James Taylor",   "Monitor Light Bar",   "$69.00",  "Cancelled",  "red",    "May 7"],
                ].each do |av, name, product, amount, status, color, date|
                  TableRow {
                    TableCell {
                      HStack(spacing: 3, align: "center") {
        ...
        Card(fluid: true) { |c|
          ...
          c.description {
            List(divided: true, relaxed: true) {
              [
                ["/images/shadcn-dashboard/products/01.jpeg", "Wireless Headphones", 1245, "$89.99"],
                ["/images/shadcn-dashboard/products/02.jpeg", "Smart Watch Pro",     890,  "$249.00"],
                ["/images/shadcn-dashboard/products/03.jpeg", "Laptop Stand",        756,  "$45.00"],
                ["/images/shadcn-dashboard/products/04.jpeg", "USB-C Hub",           620,  "$59.99"],
                ["/images/shadcn-dashboard/products/05.jpeg", "Mechanical Keyboard", 534,  "$129.00"],
                ["/images/shadcn-dashboard/products/06.jpeg", "Monitor Light Bar",   412,  "$69.00"],
                ["/images/shadcn-dashboard/products/07.jpeg", "Webcam HD",           389,  "$79.00"],
              ].each do |img, name, sold, price|
                ListItem {
                  HStack(spacing: 3, justify: "between", align: "center") {
                    HStack(spacing: 3, align: "center") {
                      ...
                      Wrapper {
                        ...
                        Text(size: "xs", color: "grey") { "#{sold} sold" }
                      }
                    }
